...
log "Processing ${binding_count} group bindings..."
log ""
...
while IFS= read -r line; do
  ...
  if [[ -n "$cloud_id" ]]; then
    log "Deleting cloud binding: group ${group_id} - role: ${role_id}"
    log "  Cloud: ${cloud_id}"
    ...
    log "Deleting folder binding: group ${group_id} - role: ${role_id}"
...
log "Done."
log "Deleted bindings for ${binding_count} groups."